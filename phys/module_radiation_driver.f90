...
CONTAINS
   ...
   SUBROUTINE radiation_driver (                                  &
              ...
              , LWF0, LWF1, LWF2, LWF3,ZI_QT8          & 
              , LWUPFD, LWDNFD, SWUPFD, SWDNFD         & 
                                                                          )
   ...
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ),                &
         OPTIONAL, INTENT(INOUT) ::            LWF0,LWF1,LWF2,LWF3
   REAL, DIMENSION( ims:ime, jms:jme ),                &
         OPTIONAL, INTENT(INOUT) ::            ZI_QT8
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ),                &
         OPTIONAL, INTENT(INOUT) ::    LWUPFD, LWDNFD, SWUPFD, SWDNFD           
...
'Can not use icloud = 3 option, missing QC or QI field.')
     ...
     lwrad_select: SELECT CASE(lw_physics)
        ...
        CASE (CRM_LWSCHEME)
             CALL wrf_debug (100, 'CALL crm_lw')

             CALL CRM_LW_RAD(                                       &
                  RTHRATEN=RTHRATEN,OLR=RLWTOA,Z_I=ZI_QT8           &
                 ,LWF0=LWF0,LWF1=LWF1,LWF2=LWF2,LWF3=LWF3           &
                 ,P8W=p8w,P3D=p,PI3D=pi,T8W=t8w                     &
                 ,Z=z,DZ8W=dz8w,RHO3D=rho                           &
...
'Can not call ETARA (2b). Missing moisture fields.')
        ...
        CASE (0)
           ...
           IF (lw_physics /= HELDSUAREZ .and. lw_physics /= CRM_LWSCHEME) THEN 
