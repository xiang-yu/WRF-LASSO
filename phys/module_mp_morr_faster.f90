...
END SUBROUTINE MORR_FASTER_INIT
...
SUBROUTINE MP_MORR_FASTER(ITIMESTEP,                       &
		...
		TKE, KZH,                               & 
               ...
               ,ITS,ITE, JTS,JTE, KTS,KTE,EFFCS,PBL     & 
               ,Zet                                     & 


                 ,crm_morr_act_opt, crm_mp_nc &
                 ,SEDFQC  ,SEDFQR    & 
                 ,QVDT_PR ,QVDT_COND & 
                 ,QCDT_PR ,QCDT_SED  &
                 ,QRDT_SED           &
                 ,RM_AER1,SIG_AER1,NA_AER1_GRD,HYGRO_AER1 &
                 ,RM_AER2,SIG_AER2,NA_AER2_GRD,HYGRO_AER2 &
                 ,RM_AER3,SIG_AER3,NA_AER3_GRD,HYGRO_AER3 &
                 ,RM_AER4,SIG_AER4,NA_AER4_GRD,HYGRO_AER4 &
                 ,SMAXACT, RMINACT    &
   ...
   INTEGER,      INTENT(IN   )    ::   ids, ide, jds, jde, kds, kde, &
                                       ims, ime, jms, jme, kms, kme, &
                                       its, ite, jts, jte, kts, kte, PBL  


   REAL, DIMENSION(ims:ime, kms:kme, jms:jme), INTENT(INOUT):: &
                          qv, qc, qr, qi, qs, qg, ni, ns, nr, nc, TH, NG, EFFCS 

   REAL, DIMENSION(ims:ime, kms:kme, jms:jme), INTENT(IN):: &
                          pii, p, dz, rho, tke, kzh, w 
   ...
   REAL, DIMENSION(kts:kte) ::                                                                & 
                            ...
                            QC1D, QI1D, QR1D,NI1D, NS1D, NR1D, QS1D, NC1D,                    & 
                            T_TEND1D,QV_TEND1D, T1D, QV1D, P1D, W1D, WVAR1D,                  &
                            EFFC1D, EFFI1D, EFFS1D, EFFR1D, DZ1D, Zet1D,                      & 
   ...
   INTEGER, INTENT(IN) :: crm_morr_act_opt
   REAL,    INTENT(IN)    :: crm_mp_nc 


   REAL, INTENT(OUT), DIMENSION(ims:ime, kms:kme, jms:jme ) ::  &
                sedfqc, sedfqr, qvdt_pr, qvdt_cond, qcdt_pr, qcdt_sed, qrdt_sed, &
                smaxact, rminact 


   REAL, DIMENSION( kts:kte ) :: sedfqcz, sedfqrz, &
                        qvdt_prz, qvdt_condz, qcdt_prz, & 
                        smaxactz, rminactz

   REAL, INTENT(IN)            :: rm_aer1, sig_aer1, hygro_aer1 
   REAL, INTENT(IN)            :: rm_aer2, sig_aer2, hygro_aer2 
   REAL, INTENT(IN)            :: rm_aer3, sig_aer3, hygro_aer3 
   REAL, INTENT(IN)            :: rm_aer4, sig_aer4, hygro_aer4 
   REAL, DIMENSION(kms:kme), INTENT(IN) :: na_aer1_grd      
   REAL, DIMENSION(kms:kme), INTENT(IN) :: na_aer2_grd      
   REAL, DIMENSION(kms:kme), INTENT(IN) :: na_aer3_grd      
   REAL, DIMENSION(kms:kme), INTENT(IN) :: na_aer4_grd      
   ...
   END DO
     ...
     if (crm_morr_act_opt .eq. 0) then 
       
     else if (crm_morr_act_opt .eq. 1) then
       
       INUM = 1 
       NDCNST = crm_mp_nc 
     else if (crm_morr_act_opt .eq. 2) then 
   ...
   if (ITIMESTEP .eq. 1) then 
     if (crm_morr_act_opt .eq. 1) then
       
       write (*,*) 'mp_morr_two_moment: Constant NC'
       write (*,*) 'mp_morr_two_moment: INUM = ', INUM
       write (*,*) 'mp_morr_two_moment: NDCNST = ', NDCNST
     else if (crm_morr_act_opt .eq. 2) then 
       write (*,*) 'mp_morr_two_moment: Prescribed aerosol S.D.'
       write (*,*) 'mp_morr_two_moment: IACT = ', IACT
       write (*,*) 'mp_morr_two_moment: ISUB = ', ISUB
     end if
  end if
   ...
   do j=jts,jte      
      ...
      call MORR_TWO_MOMENT_MICRO(QC_TEND1D, QI_TEND1D, QNI_TEND1D, QR_TEND1D,NC_TEND1D,  & 
       NI_TEND1D, NS_TEND1D, NR_TEND1D,                                                  &
       QC1D, QI1D, QS1D, QR1D,NI1D, NS1D, NR1D, NC1D,                                    & 
       ...
       QGSTEN,QRSTEN,QISTEN,QNISTEN,QCSTEN,Zet1D, &				   
	  ...
	  EFFG(I,K,j)      = EFFG1D(K)
          ...
          EFFCS(I,K,J)     = MIN(EFFC(I,K,J), 60.)  
          EFFCS(I,K,J)     = MAX(EFFCS(I,K,J),2.5)  

        sedfqc(i,k,j) = sedfqcz(k) 
        sedfqr(i,k,j) = sedfqrz(k)

        qvdt_pr(i,k,j)  = qvdt_prz(k)
        qvdt_cond(i,k,j)= qvdt_condz(k) 
        qcdt_pr(i,k,j)  = qcdt_prz(k)

        qcdt_sed(i,k,j) = QCSTEN(k)
        qrdt_sed(i,k,j) = QRSTEN(k)

        smaxact(i,k,j) = smaxactz(k)
        rminact(i,k,j) = rminactz(k) 
...
END SUBROUTINE MP_MORR_FASTER



      SUBROUTINE MORR_TWO_MOMENT_MICRO(QC3DTEN,QI3DTEN,QNI3DTEN,QR3DTEN,NC3DTEN, & 
       NI3DTEN,NS3DTEN,NR3DTEN,QC3D,QI3D,QNI3D,QR3D,NI3D,NS3D,NR3D,NC3D,         & 
       ...
       EFFC,EFFI,EFFS,EFFR,DT,                                                   &
                        ...
                        QGSTEN,QRSTEN,QISTEN,QNISTEN,QCSTEN,ZET3D, & 		   
     ...
     REAL cxr,cxi,Gzr,Gzi,Gzs,Gzg,tmpref1,tmpref2,tmpref3,tmpref4,tmpref9  
     REAL NRref,NIref,NSref,NGref,fdielec				   
   ...
   REAL DUM4,UU3,UU4,ETA3,ETA4,SM3,SM4 
   ...
   REAL, DIMENSION( kts:kte ) :: sedfqcz, sedfqrz, &
                        qvdt_prz, qvdt_condz, qcdt_prz  
   REAL, DIMENSION( kts:kte ) :: smaxactz, rminactz



   REAL :: rm_aer1, sig_aer1, hygro_aer1
   REAL :: rm_aer2, sig_aer2, hygro_aer2
   REAL :: rm_aer3, sig_aer3, hygro_aer3
   REAL :: rm_aer4, sig_aer4, hygro_aer4
   REAL, DIMENSION( kts:kte ) :: na_aer1_grd
   REAL, DIMENSION( kts:kte ) :: na_aer2_grd
   REAL, DIMENSION( kts:kte ) :: na_aer3_grd
   REAL, DIMENSION( kts:kte ) :: na_aer4_grd
 ...
 300  CONTINUE
      ...
      ELSE IF (IBASE.EQ.2) THEN
           ...
           RM1    = RM_AER1*1.E-6       
           SIG1   = SIG_AER1               
           NANEW1 = NA_AER1_GRD(k)*1.E6 
           RM2    = RM_AER2*1.E-6       
           SIG2   = SIG_AER2               
           NANEW2 = NA_AER2_GRD(k)*1.E6 
           RM3    = RM_AER3*1.E-6       
           SIG3   = SIG_AER3               
           NANEW3 = NA_AER3_GRD(k)*1.E6 
           RM4    = RM_AER4*1.E-6       
           SIG4   = SIG_AER4               
           NANEW4 = NA_AER4_GRD(k)*1.E6 
           ...
           if (RM1 .gt. 0. .and. SIG1 .gt. 1. .and. NANEW1 .gt. 0. .and. BACT1 .gt. 0.) then 
             F11 = 0.5*EXP(2.5*(LOG(SIG1))**2)
             F21 = 1.+0.25*LOG(SIG1)
             ETA1 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW1) 
             SM1  = 2./BACT1**0.5*(AACT/(3.*RM1))**1.5
             DUM1 = 1./SM1**2*(F11*(PSI/ETA1)**1.5+F21*(SM1**2/(ETA1+3.*PSI))**0.75)
           ...
           if (RM2 .gt. 0. .and. SIG2 .gt. 1. .and. NANEW2 .gt. 0. .and. BACT2 .gt. 0.) then 
             F12 = 0.5*EXP(2.5*(LOG(SIG2))**2)
             F22 = 1.+0.25*LOG(SIG2)
             ETA2 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW2)
             SM2  = 2./BACT2**0.5*(AACT/(3.*RM2))**1.5
             DUM2 = 1./SM2**2*(F12*(PSI/ETA2)**1.5+F22*(SM2**2/(ETA2+3.*PSI))**0.75)
           ...
           if (RM3 .gt. 0. .and. SIG3 .gt. 1. .and. NANEW3 .gt. 0. .and. BACT3 .gt. 0.) then 
             F13 = 0.5*EXP(2.5*(LOG(SIG3))**2)
             F23 = 1.+0.25*LOG(SIG3)
             ETA3 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW3)
             SM3 = 2./BACT3**0.5*(AACT/(3.*RM3))**1.5
             DUM3 = 1./SM3**2*(F13*(PSI/ETA3)**1.5+F23*(SM3**2/(ETA3+3.*PSI))**0.75)
           ...
           if (RM4 .gt. 0. .and. SIG4 .gt. 1. .and. NANEW4 .gt. 0. .and. BACT4 .gt. 0.) then 
             F14 = 0.5*EXP(2.5*(LOG(SIG4))**2)
             F24 = 1.+0.25*LOG(SIG4)
             ETA4 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW4)
             SM4 = 2./BACT4**0.5*(AACT/(3.*RM4))**1.5
             DUM4 = 1./SM4**2*(F14*(PSI/ETA4)**1.5+F24*(SM4**2/(ETA4+3.*PSI))**0.75)
           ...
           SMAX = 1./(DUM1+DUM2+DUM3+DUM4)**0.5
           ...
           if (RM3 .gt. 0. .and. SIG3 .gt. 1. .and. NANEW3 .gt. 0. .and. BACT3 .gt. 0.) then 
             UU3 = 2.*LOG(SM3/SMAX)/(4.242*LOG(SIG3))
             DUM3 = NANEW3/2.*(1.-DERF1(UU3))
           else
             DUM3 = 0.
           end if
           ...
           if (RM4 .gt. 0. .and. SIG4 .gt. 1. .and. NANEW4 .gt. 0. .and. BACT4 .gt. 0.) then 
             UU4 = 2.*LOG(SM4/SMAX)/(4.242*LOG(SIG4))
             DUM4 = NANEW4/2.*(1.-DERF1(UU4))
           else
             DUM4 = 0.
           end if

           
           DUM2 = (DUM1+DUM2+DUM3+DUM4)/RHO(K)  



            
            if ((NANEW1+NANEW2+NANEW3+NANEW4)/RHO(K) .lt. DUM2) then 
              print*, 'MIN((NANEW1+NANEW2+NANEW3+NANEW4)/RHO(K) .lt. DUM2' 
            end if 
            DUM2 = MIN((NANEW1+NANEW2+NANEW3+NANEW4)/RHO(K),DUM2)
            ...
            smaxactz(K) = SMAX 
            DUM4 = max(BACT1,BACT2,BACT3,BACT4)
            rminactz(K) = AACT/3.*(4./(DUM4*SMAX**2.))**(1./3.)
      ...
      IF (QI3D(K).GE.QSMALL) THEN
         ...
         LAMI(K) = (CONS12*NI3D(K)/QI3D(K))**(1./DI)
         N0I(K) = NI3D(K)*LAMI(K)
	...
	PGSACW(K) = PGSACW(K)*RATIO
      ...
      T3DTEN(K) = T3DTEN(K)+(PRE(K)                                 &
               *XXLV(K)+(PRD(K)+PRDS(K)+                            &
               ...
               (PSACWS(K)+PSACWI(K)+MNUCCC(K)+MNUCCR(K)+PIACR(K)+PIACRS(K)+   &
      ...
      QC3DTEN(K) = QC3DTEN(K)+PCC(K)
       ...
       qvdt_prz(k)   = qvdt_prz(k)  -PRE(K)   
       qvdt_condz(k) = qvdt_condz(k)-PCC(K)   
       qcdt_prz(k)   = qcdt_prz(k)  -PRA(K) & 
                                    -PRC(K)   
      ...
      ELSE IF (IBASE.EQ.2) THEN
           ...
           if (RM1 .gt. 0. .and. SIG1 .gt. 0. .and. NANEW1 .gt. 0.) then 
             F11 = 0.5*EXP(2.5*(LOG(SIG1))**2)
             F21 = 1.+0.25*LOG(SIG1)
             ETA1 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW1) 
             SM1  = 2./BACT1**0.5*(AACT/(3.*RM1))**1.5
           ...
           if (RM2 .gt. 0. .and. SIG2 .gt. 0. .and. NANEW2 .gt. 0.) then 
             F12 = 0.5*EXP(2.5*(LOG(SIG2))**2)
             F22 = 1.+0.25*LOG(SIG2)
             ETA2 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW2)
             SM2  = 2./BACT2**0.5*(AACT/(3.*RM2))**1.5
           ...
           if (RM3 .gt. 0. .and. SIG3 .gt. 0. .and. NANEW3 .gt. 0.) then 
             F13 = 0.5*EXP(2.5*(LOG(SIG3))**2)
             F23 = 1.+0.25*LOG(SIG3)
             ETA3 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW3)
             SM3 = 2./BACT3**0.5*(AACT/(3.*RM3))**1.5
           ...
           if (RM4 .gt. 0. .and. SIG4 .gt. 0. .and. NANEW4 .gt. 0.) then 
             F14 = 0.5*EXP(2.5*(LOG(SIG4))**2)
             F24 = 1.+0.25*LOG(SIG4)
             ETA4 = (ALPHA*DUM/GG)**1.5/(2.*PI*RHOW*GAMM*NANEW4)
             SM4 = 2./BACT4**0.5*(AACT/(3.*RM4))**1.5
 ...
 200     CONTINUE
	...
	DUMFNG(K) = NG3D(K)+NG3DTEN(K)*DT
      ...
      sedfqcz(k) = sedfqcz(k) + FALOUTC(K)/RHO(k)/NSTEP
      sedfqrz(k) = sedfqrz(k) + FALOUTR(K)/RHO(k)/NSTEP
 ...
 500  CONTINUE
      ...
      END IF
          ...
          IF (INUM.EQ.0.AND.IACT.EQ.3) THEN
            NC3D(K) = MIN(NC3D(K),(NANEW1+NANEW2+NANEW3+NANEW4)/RHO(K))
          END IF
      ...
      IF (QR3D(K)>1.E-14 .and. NRref>1.E-3)  tmpref1 = cxr*Gzr*tmpref9*QR3D(K)*QR3D(K)/NRref
      IF (QI3D(K)>1.E-14 .and. NIref>1.E-3)  tmpref2 = cxi*Gzi*tmpref9*QI3D(K)*QI3D(K)/NIref
      IF (QNI3D(K)>1.E-14 .and. NSref>1.E-3) tmpref3 = cxi*Gzs*tmpref9*QNI3D(K)*QNI3D(K)/NSref
      IF (QG3D(K)>1.E-14 .and. NGref>1.E-3)  tmpref4 = cxi*Gzg*tmpref9*QG3D(K)*QG3D(K)/NGref

      ZET3D(K) = tmpref1+tmpref2+tmpref3+tmpref4
      IF (ZET3D(K) >0.) THEN
	ZET3D(K) = 10.*log10((ZET3D(K)*1.E18))
      ELSE
	ZET3D(K) = -99.
      ENDIF
      ZET3D(K) = MAX(ZET3D(K),-99.)
...
END MODULE module_mp_morr_faster
